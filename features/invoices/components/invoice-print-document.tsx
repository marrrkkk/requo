    ...
    >
      <header className="flex flex-col gap-6 border-b border-border/70 pb-6">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          ...
          <div className="grid gap-3 sm:min-w-72 sm:grid-cols-2 md:grid-cols-1">
            <DetailCard label="Invoice number" value={invoice.invoiceNumber} />
            <DetailCard label="Due date" value={formatQuoteDate(invoice.dueDate)} />
          </div>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
          <div data-padding="none" className="soft-panel px-4 py-4 shadow-none print:border-border/70">
            ...
            <p className="mt-1 break-all text-sm leading-6 text-muted-foreground">
              {invoice.customerEmail ?? "No email saved"}
            </p>
          </div>
          ...
          <div data-padding="none" className="soft-panel px-4 py-4 shadow-none print:border-border/70">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <DetailRow
                label="Line items"
                value={`${invoice.items.length} ${invoice.items.length === 1 ? "item" : "items"}`}
              />
              <DetailRow
                label="Balance due"
                value={formatQuoteMoney(invoice.balanceInCents, invoice.currency)}
              />
            </div>
          </div>
        </div>
                  ...
                  invoice.items.map((item) => (
                    <tr className="border-t border-border/75" key={item.id}>
                      ...
                      <td className="px-4 py-3 text-right text-muted-foreground">
                        {formatQuoteMoney(item.unitPriceInCents, invoice.currency)}
                      </td>
                      <td className="px-4 py-3 text-right font-medium text-foreground">
                        {formatQuoteMoney(item.lineTotalInCents, invoice.currency)}
                      </td>
                    </tr>
                  ))
                ) : (
        ...
        <aside className="flex flex-col gap-4">
          <section data-padding="none" className="soft-panel px-4 py-4 shadow-none print:border-border/70">
            ...
            <div className="mt-4 flex flex-col gap-3">
              <SummaryRow
                label="Subtotal"
                value={formatQuoteMoney(invoice.subtotalInCents, invoice.currency)}
              />
              <SummaryRow
                label="Discount"
                value={`-${formatQuoteMoney(invoice.discountInCents, invoice.currency)}`}
              />
              <SummaryRow
                label={invoice.taxLabel ? `Tax (${invoice.taxLabel})` : "Tax"}
                value={formatQuoteMoney(invoice.taxInCents, invoice.currency)}
              />
              <div className="border-t border-border/70 pt-3">
                <SummaryRow
                  label="Total"
                  strong
                  value={formatQuoteMoney(invoice.totalInCents, invoice.currency)}
                />
                <SummaryRow
                  label="Paid"
                  value={formatQuoteMoney(invoice.paidInCents, invoice.currency)}
                />
                <SummaryRow
                  label="Balance due"
                  strong
          ...
          <section data-padding="none" className="soft-panel px-4 py-4 shadow-none print:border-border/70">
            ...
            <div className="mt-4 flex flex-col gap-3">
              ...
              <DetailRow label="Issued" value={formatQuoteDate(invoice.issueDate)} />
              <DetailRow label="Due" value={formatQuoteDate(invoice.dueDate)} />
      ...
      {invoice.paymentTerms ? (
        <section data-padding="none" className="soft-panel px-4 py-4 shadow-none print:border-border/70">
          ...
          <p className="mt-2 whitespace-pre-wrap text-sm leading-6">{invoice.paymentTerms}</p>
        </section>
      ) : null}
    </article>
  );
