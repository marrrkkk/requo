...
export function RecordPaymentDialog({ balanceInCents, currency, invoiceNumber, customerName, action, onOptimisticRecord, onOptimisticRevert }: { balanceInCents: number; currency: string; invoiceNumber: string; customerName: string; action: (state: PaymentActionState, formData: FormData) => Promise<PaymentActionState>; onOptimisticRecord?: (payment: OptimisticPayment) => void; onOptimisticRevert?: (key: string) => void }) {
  ...
  return (
    <ResponsiveOverlay open={open} onOpenChange={setOpen}>
      <Button type="button" onClick={handleOpen} disabled={balanceInCents <= 0}><CreditCard data-icon="inline-start" />Record payment</Button>
      <ResponsiveOverlayContent className="sm:max-w-lg">
        ...
        <form action={formAction} key={open ? idempotencyKey : "closed"} onSubmit={handleSubmit}>
          <ResponsiveOverlayBody className="flex flex-col gap-4">
            <p className="text-sm text-muted-foreground">{invoiceNumber} · {customerName}</p>
            <p className="text-sm text-muted-foreground">Remaining balance: <span className="font-semibold text-foreground">{balanceLabel}</span></p>
            ...
            <div className="grid gap-2"><Label htmlFor="payment-notes">Notes {notesRequired ? null : <span className="text-muted-foreground">(optional)</span>}</Label><Textarea id="payment-notes" name="notes" rows={3} required={notesRequired} placeholder={notesRequired ? "Describe how this payment was received." : undefined} aria-invalid={Boolean(state.fieldErrors?.notes)} /></div>
