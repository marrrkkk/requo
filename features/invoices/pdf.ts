...
import type { InvoiceDocumentData } from "@/features/invoices/documents";
...
export async function createInvoicePdf(data: InvoiceDocumentData) {
  const report = await PdfReport.create(`${data.invoiceNumber} ${data.title}`);
  ...
  report.drawWrappedText("Billed to:", {
    font: report.bodyFont,
    size: 9,
    color: report.colors.muted,
    gapAfter: 3,
  ...
  drawInfoRow("Issued:", formatQuoteDate(data.issueDate));
  drawInfoRow("Due:", formatQuoteDate(data.dueDate));
  ...
  if (data.taxInCents > 0) {
    drawTotal(data.taxLabel ? `Tax (${data.taxLabel}):` : "Tax:", formatQuoteMoney(data.taxInCents, data.currency));
  }
  ...
  drawTotal("Paid:", formatQuoteMoney(data.paidInCents, data.currency));
  drawTotal("Balance due:", formatQuoteMoney(data.balanceInCents, data.currency), true, false);

  if (data.paymentTerms?.trim()) {
    report.y -= 12;
    report.drawDivider(0, 12);
    report.drawWrappedText("Payment terms", {
      font: report.boldFont,
      size: 9,
      color: report.colors.muted,
      gapAfter: 4,
    });
    report.drawWrappedText(data.paymentTerms.trim().slice(0, 600), {
      size: 10,
      gapAfter: 4,
    });
  }
  ...
  if (data.notes?.trim()) {
    ...
    report.drawWrappedText(data.notes.trim().slice(0, 600), {
      size: 10,
      gapAfter: 4,
    });
  }
...
export function getInvoicePngFileName(data: InvoiceDocumentData) {
  return `${sanitizeStorageFileName(data.invoiceNumber, "invoice")}.png`;
}
