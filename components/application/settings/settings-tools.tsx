    ...
    <span className="relative flex size-8 shrink-0 items-center justify-center rounded-lg">
      ...
      <span
        ...
        className={cx(
          ...
          server.status === "connected" ? "bg-success" : "bg-destructive",
        )}
      />
    </span>
  );
    ...
    <Dropdown isOpen={isOpen} onOpenChange={setIsOpen}>
      <DropdownTrigger
        ...
        className={cx(
          ...
          "text-muted-foreground outline-none transition-colors duration-150 ease",
        ...
        <DropdownGroup>
          {["Show output", "Refresh tools", "Remove server"].map((label) => (
            <DropdownItem key={label} onSelect={() => setIsOpen(false)} className="px-2 py-1.5">
              <span className="truncate text-body-medium whitespace-nowrap text-foreground">{label}</span>
            </DropdownItem>
          ))}
        </DropdownGroup>
      </DropdownPopover>
...
function InlineAction({ children, onClick }: { children: ReactNode; onClick?: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cx(
        "cursor-pointer rounded-sm text-body-2-regular whitespace-nowrap text-muted-foreground",
...
function ServerRow({ server }: { server: McpServer }) {
  ...
  return (
    <div className="border-b border-border py-2.5 pr-2.5 last:border-b-0">
      <div className="flex w-full items-center gap-2.5">
        <ServerTile server={server} />
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex items-baseline gap-2">
            <p className="text-body-medium whitespace-nowrap text-foreground">{server.name}</p>
            <InlineAction>Logout</InlineAction>
          </div>
          {server.status === "connected" ? (
            <div className="flex items-center gap-1">
                <p className="truncate text-body-2-regular text-muted-foreground">{server.summary}</p>
              {server.tools && (
                <button
                  type="button"
            ...
            <div className="flex items-center gap-1">
              <p className="text-body-2-regular whitespace-nowrap text-muted-foreground">Error</p>
              <span className="text-body-2-regular text-muted-foreground">–</span>
    ...
    >
      <span
        className={cx(
          ...
          "transition-colors duration-150 ease group-hover:bg-accent",
        )}
      >
        <RiAddLine className="size-5 shrink-0 text-muted-foreground" aria-hidden />
      </span>
      <span className="flex min-w-0 flex-col">
        <span className="text-body-medium whitespace-nowrap text-foreground">New MCP Server</span>
        <span className="text-body-2-regular whitespace-nowrap text-muted-foreground">
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      ...
      <div className="flex w-full flex-col gap-2">
        <div className="flex w-full flex-col gap-0.5">
          <SettingsSectionLabel className="px-2">{scope.label} MCP Servers</SettingsSectionLabel>
            <p className="w-full px-2 text-body-2-regular text-muted-foreground">
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <div className="flex w-full flex-col items-center gap-3 rounded-2xl bg-muted px-6 py-8">
          <div className="flex flex-col items-center gap-1 text-center">
            <p className="text-body-medium text-foreground">No Team MCP Servers</p>
            <p className="max-w-[360px] text-body-2-regular text-muted-foreground">
