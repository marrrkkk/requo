  ...
  return (
    <div className="relative size-8">
      <span
        aria-hidden
        className={cx(
          "absolute inset-y-0 bg-primary/10 transition-[opacity,border-radius] duration-100 ease-out",
      ...
      >
        ...
        />
        <span className={cx("relative text-body-medium", isEdge ? "text-primary-foreground" : "text-foreground", isDisabled && "text-muted-foreground")}>
          {formattedDate}
        </span>
      </div>
    </div>
...
export function MonthPanel({
  ...
  return (
      <div className={bare ? "w-[296px] shrink-0" : "w-[326px] shrink-0 rounded-2xl bg-card p-[15px] shadow-xs"}>
      <div className="flex flex-col gap-5">
        {!hideHeader && (
          <div className="flex items-center justify-between">
              {showPrev ? (
                <RACButton
                  slot="previous"
                  className="flex size-4 cursor-pointer items-center justify-center rounded-[3px] text-muted-foreground outline-none transition-colors duration-150 ease hover:bg-accent"
                >
                <ChevronLeft16 />
              </RACButton>
            ) : (
            ...
            <span className="flex-1 text-center text-body-medium text-foreground">{title}</span>
              {showNext ? (
                <RACButton
                  slot="next"
                  className="flex size-4 cursor-pointer items-center justify-center rounded-[3px] text-muted-foreground outline-none transition-colors duration-150 ease hover:bg-accent"
        ...
        >
          <CalendarGridHeader>
            {(day) => (
              <CalendarHeaderCell className="size-6 pb-0 text-center text-body-medium text-muted-foreground">
                {day.slice(0, 2)}
              </CalendarHeaderCell>
            )}
          </CalendarGridHeader>
...
export function DateChipInput({
  ...
  return (
    <input
      ...
      aria-label={label}
      className="w-[104px] rounded-md border border-border bg-card px-2 py-2 text-body-medium text-foreground shadow-xs outline-none transition-colors duration-100 ease-out focus-visible:border-ring"
    />
  );
}
...
export const triggerButtonClassName = cx(
  "inline-flex shrink-0 cursor-pointer items-center gap-0.5 rounded-md border border-border bg-card p-2 shadow-xs outline-none",
