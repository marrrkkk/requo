 ...
 *
 * - Panel: card surface, 1px border, radius 16, p 10, overlay shadow.
 * - Appear animation: 150ms fade + scale-95 + 2px blur in and out.
 * - Rows: rounded-md, accent on hover and on the selected
...
export function DropdownGroup({ label, className, children }: DropdownGroupProps) {
  return (
    ...
    <div className={cx("flex w-full flex-col gap-1.5", label && "pt-1", className)}>
      {label && <span className="pl-2 text-body-medium text-muted-foreground">{label}</span>}
...
export function DropdownDivider({ className }: { className?: string }) {
  return <div className={cx("-mx-2.5 my-1.5 h-px shrink-0 bg-border", className)} />;
}
