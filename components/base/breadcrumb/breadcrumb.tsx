      ...
      <ol className="flex items-center gap-2.5 px-1">
        {items.map((item, index) => (
          <Fragment key={item.key ?? index}>
            {index > 0 && (
              <ChevronRightSmall className="size-3 shrink-0 text-muted-foreground" aria-hidden />
            )}
            {item}
          </Fragment>
        ))}
...
export function BreadcrumbItem({
  ...
  if (current) {
    return (
      <li
        aria-current="page"
        className={cx(
          "flex items-center gap-1.5 text-caption-1-medium whitespace-nowrap text-foreground",
          className,
        )}
      >
        {content}
  ...
  const interactiveClass = cx(
    "-mx-1 flex items-center gap-1.5 rounded-md px-1 py-0.5 text-caption-1-medium whitespace-nowrap text-muted-foreground",
