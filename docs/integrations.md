...
## Invoice payment providers (platform links / BYO keys)

- No SDK packages — plain `fetch` in `lib/payments/providers/{paymongo,stripe,paypal}/*` behind the `PaymentProviderAdapter` interface (`lib/payments/types.ts`); feature code never imports provider APIs. Registry: `lib/payments/adapters.ts`.
- Two connect modes, one row shape (Settings → Integrations, owner-only): **platform link** (Stripe Connect — Requo's platform key + `Stripe-Account` header for direct charges, owner authorizes via Stripe-hosted onboarding, `auth_mode = platform`, `provider_account_id` set, no merchant secret stored) and **BYO keys** (secrets AES-256-GCM in `credentials_ciphertext`, `lib/payments/secret-box.ts`, key `PAYMENT_CREDENTIALS_KEY`, server-only). `payment_provider_connections` is unique on business/provider/`test|live`; `provider_connection_attempts` holds single-use 30-min flow pointers. Requo is never the merchant of record.
- Platform linking: `lib/payments/providers/stripe/{platform,connect}.ts` + `features/payment-providers/mutations.ts` (attempt → start → callback/refresh) + `app/api/payments/stripe/connect/{return,refresh}`. `status` (`onboarding/action_required/ready/revoked`) gates operations — `ready` is derived from the provider account (`charges_enabled` + `details_submitted` + empty `currently_required`), never from a finished callback. Events are account-matched, so one connected account cannot credit another business.
- Checkout links: pending `payments` row + idempotency key first, then provider-hosted session (`/pay/return` is informational only). Canonical money: PayMongo `pay_…`, Stripe PaymentIntent, PayPal capture. Async/unpaid completions and order approvals never count; PayPal approval triggers a server-side capture in the worker.
- Webhooks: `app/api/payments/webhooks/{paymongo,stripe,paypal}/[connectionId]/route.ts` (shared `lib/payments/route-handler.ts`) → raw-body signature verify (platform secret for linked rows) → atomic `payment_events` insert → fast 2xx → Inngest `requo/payment.event-received` reconciles monotonically (snapshot truth, cumulative refunds, net-paid invoice recalc). Refresh (`refreshProviderPayment`) reuses the write path; refunds are idempotent attempts confirmed by webhook.
- Env: `PAYMENT_CREDENTIALS_KEY` (base64 32-byte); optional `STRIPE_PLATFORM_SECRET_KEY` + `STRIPE_PLATFORM_WEBHOOK_SECRET` (both required together — absent means platform mode is inert and BYO still works). Per-connection secrets live in the DB, never in env. Setup + test → live: `docs/setup/payments.md`. Rules: ADR-012, ADR-013.
- Failure: bad signature → 400 no insert; unusable/unknown events → failed/ignored + 2xx (no retry storm); enqueue failure → 5xx; stale `processing` events resubmit via 15-min cron. Provider API failure marks the pending row `failed` (never counts).

## Polar billing (SaaS subscriptions, merchant of record)
...
- Events (`lib/inngest/events.ts`): `requo/inquiry.qualified` → AI draft; `requo/knowledge.file-uploaded` → chunk+embed (concurrency 5/business, 2 retries); push events (inquiry-received, quote-sent, quote-response, invoice-*); `requo/quotes.enable-auto-follow-up`; `requo/payment.event-received` → provider payment reconciliation (retries 3). Cron: follow-up reminders, auto-follow-ups, quote-viewed/expiring, auto-archive, expire quotes/subscriptions, analytics rollup/digest/reports/benchmarks, session expiry, token-log cleanup, stale payment-event resubmission (every 15 min), embedding backfill (`cron-embedding-backfill`, hourly, `concurrency: limit 1` — repairs `embedding IS NULL` rows).
