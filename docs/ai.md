...
2. Orchestrator: load session → `sanitizeAiInput` → persist user turn → advance state (Agent: qualification extraction; Assistant: title derivation) → build prompt from history **after** persist.
   - Ephemeral chat attachments (both surfaces, `lib/ai/chat-attachments.ts`): files ride the JSON body as data URLs (never message parts), decoded in the route, then validated → daily plan bucket (`assistantFileUploadsPerDay` / `agentFileUploadsPerDay`: 5/20/50) → extracted via `extractKnowledgeText` (PDF/DOCX/CSV/TXT/MD, images rejected) → `sanitizeMemoryContent` → truncated to a per-turn char budget (Assistant 10k, Agent 5k) → appended as `[Attached file: …]` blocks to the user turn. Bytes are discarded; only the text plus an `attachmentCount` message-metadata marker persist.
