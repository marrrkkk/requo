...
## Amendment A1 (2026-09-16) — `/admin` gate moved into `proxy.ts`

This amendment records a change made after the audit date. The audit body above is otherwise left as written, because it is a point-in-time record.

**What changed.** `proxy.ts` now performs one auth check: an `/admin` gate that resolves the session and answers a signed-in non-admin with a real `403` before the route renders. Previously it did no auth, and the audit's "no middleware-only authz" line remains true — the proxy is not the only boundary. `requireAdminUser()` still re-checks every admin page, query, mutation, and action, and every server entry still re-validates independently.

**Why it is not middleware-only authz.** The gate reads Better Auth's session cookie cache and can lag the database by up to `session.cookieCache.maxAge` (300s), so on its own it would admit a just-demoted admin. It exists for a status-code reason the audit could not have anticipated: `app/admin/layout.tsx` must render synchronously (its pages export `instant = true`), so under `cacheComponents` the admin shell is flushed before the layout's Suspense-wrapped gate resolves. Any later rejection — `forbidden()` included — arrives after the chrome has been sent, with the status already committed to `200`. That is the flash the gate removes.

**Audit findings unaffected.** SEC-001, SEC-002, SEC-003, SEC-004 and the remainder of the register are untouched by this change. The tenant boundary (`getBusinessActionContext` + `businessId`-scoped predicates) is unchanged, and the proxy still resolves no business context.

**One audit observation is now recorded in code.** The V2/V6/V7 note already flagged "cookie-cache `role` staleness after demotion". That is confirmed and documented in `features/admin/access.ts`: the `role === "admin"` fast path returns **without** reading the database, so a demoted or banned admin keeps console access until the cookie cache lapses. `docs/authentication.md` now states this explicitly rather than describing the gate as DB-authoritative.

**New surface introduced.** The gate adds `features/admin/forbidden-page.ts` (static HTML, no interpolated input) and `app/forbidden.tsx`, and enables `experimental.authInterrupts` so the residual database-check rejection renders a `403` boundary instead of an unhandled error. That flag is experimental and build-time-inlined; `next.config.ts` records the dependency and its implications.
