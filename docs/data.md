...
- **No pgvector.** `business_memories.embedding` and `business_knowledge_chunks.embedding` are `jsonb number[]`; cosine similarity computed app-side (`features/memory/retrieval.ts`). Works because per-business memory counts are small.
