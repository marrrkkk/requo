...
- **Follow-up task** (`follow_ups`, `features/follow-ups/`): owner reminder linked to inquiry and/or quote, `pending → completed|skipped`, recurrence + snooze + `reminderSentAt`. Send mode `manual` (owner sends it) vs `automatic` (emailed to the customer at `dueAt` by the hourly `cron-follow-up-auto-send` job, then marked completed with `follow_up.auto_sent`; email channel only, requires a customer email). Surfaced in inbox, dashboard, and Inngest reminder job.
