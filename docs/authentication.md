...
- Admin console is role-based (`user.role = "admin"`, normal Requo login; allowlist `ADMIN_EMAILS` only for the bootstrap script) behind the admin subdomain (`lib/admin/subdomain-config.ts`).

## Middleware (`proxy.ts`) — no auth

`proxy.ts` never checks sessions. It does: admin-subdomain rewrite (except `/api/`, `/_next/`), `X-Robots-Tag: noindex` for non-public routes, `/` + `Accept: text/markdown` → `/api/public/markdown`, legacy `/account/profile|security` → `/{slug}/settings/profile`, and the `activeBusinessSlug` httpOnly cookie. All auth lives in layouts, actions, and route handlers.
